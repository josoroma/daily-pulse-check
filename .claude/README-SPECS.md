...
> How [SPECS.md](../SPECS.md) epics, stories, and tasks map to the colocated route architecture defined in [README-ARCHITECTURE.md](README-ARCHITECTURE.md).
>
> Use this file when:
>
> - The `plan-item` agent needs to know where a story's files live
> - The `implement-item` skill needs to choose the correct route folder
> - The `review-item` agent needs to confirm a story's files match its declared scope
>
> SPECS.md currently defines **14 epics (E1–E14)** and **46 user stories**. When SPECS.md changes, update this file alongside it (or use `/update-specs`).
...
| **US-6.3**: Ask AI About Market Concepts           | `app/dashboard/insights/`                        | `_components/chat.tsx`, `lib/ai/learning-assistant.ts`                                       |
| **US-6.4**: Chain of Thought Reasoning Display     | `app/dashboard/insights/`, `lib/ai/`             | NDJSON stream, reasoning toggles in all AI components                                        |
...
| **US-7.3**: Notification Delivery Channels         | `lib/notifications/`                             | `dispatcher.ts`, `telegram.ts`, `email.ts`                                                   |
...
| **US-9.3**: Tax-Relevant Export (CR)               | `app/dashboard/analytics/`                       | `_utils.ts`, `_components/`                                                                  |
...
| **US-12.1**: Apply shadcn/ui Luma Design Style     | `app/`                                           | `globals.css` — CSS variables, radius, shadows                                               |
...
| **US-14.1**: Demo User with 15-Day Historical Data | `supabase/`                                      | `seed.sql`, `docs/technical/demoUserSeedDemo.md`                                             |
...
3. ✅ Shared utilities (3+ routes) live in `lib/<domain>/`
4. ✅ `__tests__/` exists with tests for `_schema.ts` and `_utils.ts`
5. ✅ Gherkin scenarios are satisfied before marking the story `[x]`
6. ✅ SPECS.md status markers and the Progress Summary table reflect reality

When reviewing with `review-item`, also verify:

7. ✅ Files outside this mapping have a documented reason in the latest `docs/agents/item-implementation-plan-*.md`
8. ✅ No route-specific code leaked into `lib/` and no shared code is duplicated across routes
